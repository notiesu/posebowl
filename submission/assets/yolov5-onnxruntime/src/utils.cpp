...
void utils::createImageBatch(const std::string& imageDir, std::vector<std::__fs::filesystem::path>& imagePaths)
{
...
void utils::writeToCSV(std::vector<Detection> result, std::string imageID, std::ofstream &ostream) {
    for (int i = 0; i < result.size(); i++) {
        ...
        int min_X = detection.box.x - detection.box.width;
        int min_Y = detection.box.y - detection.box.height;
